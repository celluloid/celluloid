...
shared_examples "Celluloid::Actor examples" do |included_module, task_klass|
  ...
  it "supports synchronous calls with blocks" do
    ...
    expect(block_executed).to be_true
  end
  ...
  it "detects recursion" do
    ...
    expect(actor1.recursion_test).to be_false
    expect(actor1.recursion_test(actor2)).to be_true
  end
  ...
  it "properly handles respond_to with include_private" do
    ...
    expect(actor.respond_to?(:zomg_private)).to be_false
    expect(actor.respond_to?(:zomg_private, true)).to be_true
  end
  ...
  it "allows an actor to call private methods asynchronously" do
    actor = actor_class.new "Troy McClure"
    actor.call_private
    expect(actor.private_called).to be_true
  end
  ...
  context :exceptions do
    ...
    it "includes both sender and receiver in exception traces" do
      ...
      expect(ex.backtrace.grep(/`sender_method'/)).to be_true
      expect(ex.backtrace.grep(/`receiver_method'/)).to be_true
    end
  ...
  context :linking do
    ...
    it "links to other actors" do
      @kevin.link @charlie
      expect(@kevin.monitoring?(@charlie)).to be_true
      expect(@kevin.linked_to?(@charlie)).to  be_true
      expect(@charlie.monitoring?(@kevin)).to be_true
      expect(@charlie.linked_to?(@kevin)).to  be_true
    end
    ...
    it "unlinks from other actors" do
      ...
      expect(@kevin.monitoring?(@charlie)).to be_false
      expect(@kevin.linked_to?(@charlie)).to  be_false
      expect(@charlie.monitoring?(@kevin)).to be_false
      expect(@charlie.linked_to?(@kevin)).to  be_false
    end
  ...
  context :signaling do
    ...
    it "sends values along with signals" do
      ...
      expect(obj.send_signal(:foobar)).to be_true
  ...
  context :exclusive do
    ...
    it "knows when it's in exclusive mode" do
      expect(subject.check_not_exclusive).to be_false
      expect(subject.check_exclusive).to be_true
    end

    it "remains in exclusive mode inside nested blocks" do
      expect(subject.nested_exclusive_example).to be_true
    end
  end
